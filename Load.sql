...
CREATE TABLE team
  ...
  coach_name VARCHAR(20) REFERENCES coach(name),
  coach_salary NUMBER(8,2) REFERENCES coach(salary)
);
...
CREATE TABLE player
  ...
  rebs NUMBER(3,1),
  asts NUMBER(3,1),
  team_name VARCHAR2(20) REFERENCES team(name)
);
...
CREATE TABLE plays_in
  (name VARCHAR2(20) REFERENCES player(name),
  dt TIMESTAMP REFERENCES game(dt) DISABLE
);
